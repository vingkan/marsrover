	...
	'arcgauge': {
		...
		activate: function(id, model, interface){
			...
			var config = {
				...
				type: interface.config.type || 'default'

			}
			function getAngle(value){
				return (config.max - value); // Dirty: watch out!
			}
			$('#arcgauge-' + id).roundSlider({
				...
				drag: (e) => {
					...
					if(config.type === 'range'){
						var range = e.value.split(',').map(val => getAngle(parseFloat(val, 10)));
						console.log(range)
					}
					else{
						input = getAngle(e.value);
					}
				}
			});
			var button = panel.getElementsByTagName('button')[0];
