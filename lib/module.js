...
var InterfaceGenerator = {
	...
	'numberpad': {
		render: function(id, interface){
			...
			for(var r = 0; r < rows; r++){
				...
				for(var c = 0; c < cols; c++){
					...
					row += '<button class="numbutton active" data-row="' + r + '" data-col="' + c + '" data-number="' + n + '">' + n + '</button>'
				}
		...
		activate: function(id, model, interface){
			var panel = document.getElementById(id);
			var input = 0;
			var activation = interface.activation || 'remaining';
			...
			forEachButton(function(nb){
				nb.addEventListener('click', function(e){
					var number = parseInt(e.target.dataset.number, 10);
					input += number;
					...
					var pivotOnRow = true;
					switch(activation){
						case 'any': forEachButton(function(nb){
								nb.disabled = false;
							});
							break;
						case 'adjacent': forEachButton(function(nb){
								var r = parseInt(nb.dataset.row, 10);
								var c = parseInt(nb.dataset.col, 10);
								adjRow = Math.abs(col - c) <= 1 && row === r;
								adjCol = Math.abs(row - r) <= 1 && col === c;
								nb.disabled = !xor(adjRow, adjCol);// && !nb.disabled; // May need to store used ones in list
							});
							break;
						case 'row': forEachButton(function(nb){
								var r = parseInt(nb.dataset.row, 10);
								var c = parseInt(nb.dataset.col, 10);
								sameRow = (row === r);
								nb.disabled = !sameRow;
							...
							break;
						case 'column': forEachButton(function(nb){
								var r = parseInt(nb.dataset.row, 10);
								var c = parseInt(nb.dataset.col, 10);
								sameCol = (col === c);
								nb.disabled = !sameCol;
							...
							break;
						case 'alternate': forEachButton(function(nb){
								var r = parseInt(nb.dataset.row, 10);
								var c = parseInt(nb.dataset.col, 10);
								sameRow = (row === r);
								sameCol = (col === c);
								nb.disabled = !(pivotOnRow ? sameRow : sameCol);
								pivotOnRow = !pivotOnRow;
							});
							break;
						case 'value': forEachButton(function(nb){
								var n = parseInt(nb.dataset.number, 10);
								sameNumber = (number === n);
								nb.disabled = sameNumber;
							});
							break;
						case 'remaining':
							// Do nothing
					...
					e.target.disabled = true; // Disable self at the end
				});
			});
