...
gref.on('value', function(snapshot){
	...
	GLOBAL.WIN = val.win || 7;
	GLOBAL.LOSE = val.lose || 5;
	GLOBAL.LIMIT = val.limit || 13;
});
...
var REDUCERS = {
	...
	setup_controls: (state, data) => {
		...
		console.warn('check order:', state.players);
		var sortedPlayers = state.players.sort((a, b) => {
			return a.timestamp - b.timestamp;
		});
		console.warn('check order:', sortedPlayers);
		state.players = sortedPlayers.map((player) => {
			return player.id;
		});
		console.warn('check order:', state.players);
