var MISSION_ID = localStorage.getItem('mars_mission_id');
...
var STATE = {
	id: localStorage.getItem('mars_player_id'),
	data: {},
...
function createMission(key){
	...
	setup.then(function(done){
		var node = db.ref('missions/' + key + '/controls').push({
			'panel-1': {interface: false},
			'panel-2': {interface: false},
			'panel-3': {interface: false},
			'panel-4': {interface: false}
		});
		var playerId = node.key;
		localStorage.setItem('mars_player_id', playerId);
	});
}
